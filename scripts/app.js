function init() {
  ...
  function createNewShape() {
    const shape =
      tetriminos[Math.round(Math.random() * (tetriminos.length - 1))];
    let makeShape;
    switch (shape) {
      case "i":
  ...
  function fall(makeShape) {
    ...
    const timerId = setInterval(() => {
      ...
      if (
        Math.max(...makeShape.dimensions) > cells.length - 13 ||
        makeShape.dimensions.some((element) =>
          cells[element].classList.contains("occupied")
        )
      ) {
        ...
        if (
          makeShape.dimensions.some((element) =>
            cells[element].classList.contains("top")
          )
        ) {
          gameOver();
          return;
  ...
  function clearLine() {
    ...
    const blockedArray = [];
    blockedCells.forEach((element) =>
      blockedArray.push(parseFloat(element.textContent))
    );
    const rowCheck = [];
  ...
  function blockedRowsDown() {
    ...
    const blockedCells = [];
    blockedDivs.forEach((element) =>
      blockedCells.push(parseFloat(element.textContent))
    );
    ...
    for (let i = 0; i < rowStart.length; i++) {
      ...
      blockedRow = blockedRow.filter((element) =>
    ...
    for (let i = blockedRowArrays.length - 1; i >= 1; i--) {
      ...
      while (difference >= 13) {
        blockedRowArrays[i - 1] = blockedRowArrays[i - 1].map(
          (cell) => cell + 12
        );
  ...
  function rotateTetriminos(event) {
    ...
    if (event.keyCode === 32) {
      if (gameInMotion === false) startGame();
      gameInMotion = true;
    }
  }
  ...
  function tetriminoDown(event) {
    ...
    if (
      newTetrimino.dimensions.some((element) =>
        cells[element].classList.contains("occupied")
      )
    ) {
      newTetrimino.removeShape();
